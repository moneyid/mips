...
Covers the send-money flow: currency and chain discovery, payment requests, payment instructions, metadata, comments, payer identity, currency conversion, and success actions.
...
| `symbol` | string | Yes | Display symbol (e.g., `"$"`, `"Ξ"`) |
...
The service returns the fields required for the on-chain transfer.
...
The wallet displays "Pay $5.00 to CoffeeShop - Order #42". The customer picks a currency and chain and confirms.
